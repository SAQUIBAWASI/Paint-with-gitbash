...
const AllServices = () => {
  return (
    <div className="min-h-screen px-4 py-10 bg-gray-100 sm:px-6 lg:px-8">
      ...
      <p className="max-w-3xl mx-auto mt-4 text-base text-center text-gray-700 sm:text-lg">
        Discover our range of expert painting and cleaning solutions tailored to enhance your space.
      </p>

      {/* Services List */}
      <div className="grid max-w-6xl gap-6 mx-auto mt-12 sm:grid-cols-2 lg:grid-cols-3">
        {[
          {
          ...
          <div
            key={index}
            className="p-6 text-center transition-transform transform bg-white rounded-lg shadow-md hover:scale-105 hover:shadow-lg"
          >
            <h3 className="text-lg font-semibold text-gray-700 sm:text-xl">{service.title}</h3>
      ...
      <div className="mt-10 text-center">
        <Link to="/">
          <button className="px-6 py-3 text-base font-semibold text-white transition duration-300 bg-gray-800 rounded-lg sm:px-8 sm:text-lg hover:bg-gray-900 hover:shadow-lg">
            ⬅ Go Back
          </button>
        </Link>
      </div>
